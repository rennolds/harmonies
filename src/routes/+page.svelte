<script>
  ...
  // Check if we're playing a custom puzzle
  const isCustomPuzzle = data && data.customPuzzle;
  const customPuzzle = data?.customPuzzle;
  const puzzleNotFound = data?.puzzleNotFound;

  // Helper: use local state for both archive mode and custom puzzles
  const useLocalState = data?.isArchiveMode || isCustomPuzzle;
  ...
  // Check if we're viewing an archived puzzle or a custom puzzle
  const isArchiveMode =
    (selectedDate && selectedDate !== actualToday) || isCustomPuzzle;
  ...
  // If we have a custom puzzle, use that instead of looking up from gameBoards
  let todayBoard;
  if (isCustomPuzzle) {
    todayBoard = customPuzzle.puzzle_data;
  } else {
    todayBoard = gameBoards[todaysDate];
  }
  ...
  // Default celebration GIF for when no custom one is provided
  const defaultGameoverGif =
    "https://media.giphy.com/media/g9582DNuQppxC/giphy.gif";
  const src = board["gameoverGif"] || defaultGameoverGif;
  if (board["gameoverGif"] == "" && todayBoard && !isCustomPuzzle) {
    console.debug(`Gameover gif is missing for ${todaysDate}`);
    if (browser) {
      sendError(`Harmonies is missing a gameover gif for ${todaysDate}`);
    }
  }
  const playlist = board["playlist"] || "";
  if (playlist == "" && todayBoard && !isCustomPuzzle) {
  ...
  if (isArchiveMode || isCustomPuzzle) {
    ...
    // (Skip this check for custom puzzles since they don't use completedDays tracking)
    const isCompleted = !isCustomPuzzle && $completedDays.includes(displayDate);
    ...
    if ($todaysProgressDate !== actualToday && !isTodayCompleted) {
      ...
      if (
        (useLocalState
          ...
          : $clearedCategories.length) == 4 &&
        (useLocalState ? localMistakeCount : $mistakeCount) < 4
      ) {
        remainingElements = [];
        gameoverStore.set({
          isOver: true,
      ...
      if (
        ...
        const allClearedElements = (
          useLocalState ? localClearedCategories : $clearedCategories
        )
          .map((category) => category.elements)
          .flat();
  ...
  function shareResult() {
    ...
    // Use different header for custom puzzles
    var header = isCustomPuzzle
      ? "harmonies 🎧\n\n"
      : "harmonies #" + harmonyNumber + "🎧\n\n";

    let grid = "";

    const historyToUse = useLocalState ? localGuessHistory : $guessHistory;
    ...
    // Use different link for custom puzzles
...
<main>
  ...
  {#if puzzleNotFound}
    <div class="puzzle-not-found">
      <h1>Puzzle Not Found</h1>
      <p>This puzzle doesn't exist or may have been removed.</p>
      <a href="/" class="back-home-btn">Play Today's Harmonies</a>
    </div>
  {:else if !hideOverlay}
    <div in:slide={{ delay: 500 }} class="gameover-overlay">
      ...
      {#if isCustomPuzzle}
        <a href="/"
          ><button style="background-color: #BA81C2;" class="results-button"
